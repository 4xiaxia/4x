...
/**
 * 默认密码（当pwd文件不存在时使用）
 * 安全建议：生产环境应通过环境变量或pwd文件设置密码，避免使用默认值
 */
...
export async function readPasswordFile() {
    ...
    try {
        ...
        // 如果密码文件为空，提示用户设置密码
        if (!trimmedPassword) {
            logger.warn('[Auth] Password file is empty, please set a password in configs/pwd');
        ...
        if (error.code === 'ENOENT') {
            if (!DEFAULT_PASSWORD) {
                logger.error('[Auth] Password file does not exist and no DEFAULT_PASSWORD environment variable set');
                logger.error('[Auth] Please either: 1) Create configs/pwd file, or 2) Set DEFAULT_PASSWORD environment variable');
            } else {
